...
- run `uv run ruff check .` and `uv run ruff format .` (always on the whole repo, never on single files) after each new feature is implemented, and fix all findings
